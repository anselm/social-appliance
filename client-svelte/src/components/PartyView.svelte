<script lang="ts">
  ...
  async function loadRecentPosts() {
    ...
    loading = true
    try {
      const posts = await userApi.getUserPosts(entity.auth, 5)
      recentPosts = posts
    } catch (error) {
...
<div class="max-w-3xl">
  ...
  <div class="mb-8">
    <div class="flex items-start gap-6 mb-6">
      ...
      <div class="flex-1">
        <h1 class="text-2xl font-bold mb-2">{entity.title || entity.slug || 'Untitled'}</h1>
        {#if entity.slug}
          <p class="text-xs text-white/40 font-mono mb-3">{entity.slug}</p>
        {/if}
        {#if entity.content}
